...
public class SwaggerConfig {
	...
	public Docket swaggerConfiguration() {
		return new Docket(DocumentationType.SWAGGER_2).select().paths(PathSelectors.regex("/supplier.*"))
				.apis(RequestHandlerSelectors.basePackage("com.retail.platform")).build().apiInfo(apiDetails());
	}

	private ApiInfo apiDetails() {
		return new ApiInfo("Suppilier API", "Supplier service for retail platform", "v1.0", "termsOfServiceUrl",
